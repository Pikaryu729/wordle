...
class Solver:
    def __init__(self, wordle: Wordle):
        self.wordle = wordle
        self.allowed_words = wordle.allowed_words.copy()  # Make a copy to avoid modifying original
        self.num_guesses = 0
    ...
    def filter_words_by_response(self, guess: str, response: tuple) -> list[str]:
        ...
        """
        return [word for word in self.allowed_words if self.check_guess(guess, word) == response]
    
    def get_new_word_list(self, guess: str) -> list[str]:
        """Update word list based on the response to the current guess"""
        letter_statuses = self.check_guess(guess, self.wordle.secret_word)
    ...
    def get_best_guess(self) -> str:
        ...
        entropy_table = {guess: self.get_guess_entropy(guess) for guess in self.allowed_words}
        return max(entropy_table, key=entropy_table.get)

    
    def get_valid_input(self):
        while True:
            letter_statuses = Prompt.ask(
                "Enter list of numbers: 0 - grey, 1 - yellow, 2 - green"
            )
    
    def solve_as_you_go(self):
        guess = "tares" # starting guess
        while self.check(guess) != [2, 2, 2, 2, 2]:
            self.console.print(f"Best Guess: {self.get_best_guess()}")
            Prompt.ask("Enter list of numbers: 0 - grey, 1 - yellow, 2 - green")
    ...
    def solve(self) -> str:
        ...
        while guess != self.wordle.secret_word:
            ...
            self.allowed_words = self.get_new_word_list(guess)
